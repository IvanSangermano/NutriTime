...
export const HealthForm = () => {
  const { actionInProgress, selectedHealth, isLoading, error } = useSelector(
    (state) => state.healths
  );
  ...
  const optionsSex = [{ sex: 'Male' }, { sex: 'Female' }];

  const optionsStage = [{ stage: 'Volume' }, { stage: 'Definition' }];
        ...
        <Form
          ...
          initialValues={
            selectedHealth
              ? {
                  ...selectedHealth,
                  day: selectedHealth.day.split('T', 1)?.[0],
                }
              : initialState
          }
        >
                ...
                <Field name={'userId._id'} validate={required}>
                  {({ input }) => (
                    <ComboBoxInput
                      ...
                      optionsKey={'userName'}
                      optionsValue={'_id'}
                      disabled={!(actionInProgress != CONSULT)}
                    />
                  )}
                </Field>
              </div>
